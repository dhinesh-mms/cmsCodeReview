...
class Soap6 extends Soap5
{
    ...
    private function addDisplayEvent(SanitizerInterface $sanitizedFaultAlert)
    {
     ...
     /**
     * Returns a string containing the Settings files xml for the requesting display
     * @param string $serverKey The Server Key
     * @param string $hardwareKey Display Hardware Key
     * @return string $settingsXml Xml Formatted String
     * @throws \SoapFault
     */
    
    function SettingFiles($serverKey, $hardwareKey): string
    {
        return $this->doSettingFiles($serverKey, $hardwareKey);
    }
}
