...
class Soap
{
    ...
    protected function doRequiredFiles(
        ...
        try {
            ...
            foreach ($sth->fetchAll() as $row) {
                ...
                $path_url = $root_path.'images/'. $path;
                //$path_url = 'http://192.168.4.65/cms412/images/'. $path;
               
                $file->setAttribute("fileUrl", $path_url);
        ...
        foreach ($layouts as $layoutId) {
            try {
                ...
                $xlf_url = $root_path.'/images/'. $layoutId.'.xlf';
                //$xlf_url = 'http://192.168.4.65/cms412/images/'. $layoutId.'.xlf';
                $file->setAttribute("fileUrl", $xlf_url);
    ...
    protected function doSchedule($serverKey, $hardwareKey, $options = [])
    {
        ...
        try {
            ...
            $default->setAttribute("scheduleid", $defaultLayoutId);
    ...
    protected function doSubmitStats($serverKey, $hardwareKey, $statXml)
    {
        ...
        foreach ($document->documentElement->childNodes as $node) {
            ...
            $today = Carbon::now();
           if(!$fromDt->isSameDay($today)){
               $fromDt = $today->setTime($fromDt->hour,$fromDt->minute,$fromDt->second);
            }       

            if(!$toDt->isSameDay($today)){
              $toDt = $today->setTime($toDt->hour,$toDt->minute,$toDt->second);
            }
    ...
    protected function doMediaInventory($serverKey, $hardwareKey, $inventory)
    {
        ...
        foreach ($fileNodes as $node) {
            ...
            try {
                ...
                $requiredFile->bytesRequested = $node->getAttribute('size');

                // If this item is a 0 then set not complete
                if ($complete == 0) {
                        $mediaInventoryComplete = 2;
                        $requiredFile->bytesRequested = 0;
                }

                $requiredFile->save();
                  // File complete?
                // $complete = $node->getAttribute('complete');
                // if ($complete == 5 || $complete == 1){
                //     $requiredFile->complete = 1;
                // }else{
                //     $requiredFile->complete = $complete;
                // }
                // if($complete==1 || $complete == 5){
                //   $requiredFile->bytesRequested = $requiredFile->size;  
    ...
    private function addDependency(
        ...
        //if($dependency->size > 0){
            ...
            //$path_url = $root_path.'images/assets/'. $path;
            // $path_url = 'http://192.168.4.65/cms412/images/assets/'. $dependency->fileType;
    ...
    protected function collectionIntervalWithOffset(int $collectionInterval): int
    {
     ...
     // setting File API
      ...
      protected function doSettingFiles($serverKey, $hardwareKey): string
    {
        $this->logProcessor->setRoute('SettingFiles');
        ...
        // Sanitize
        $server_Key = $sanitizer->getString('serverKey');
        $hardwareKey = $sanitizer->getString('hardwareKey');


        if($this->getConfig()->getSetting('SECURITY_KEY_MODULE')=='On'){
            $serverKey = NewAES::decryption($server_Key);
        }else{
        ...
        // Check the cache
        $cache = $this->getPool()->getItem($this->display->getCacheKey() . '/settingFiles');
        $cache->setInvalidationMethod(Invalidation::OLD);
        ...
        if ($cache->isHit()) {
            $this->getLog()->info('Returning setting files from Cache for display ' . $this->display->display);
        ...
        $licence_date = $this->getConfig()->getSetting("LICENCE_EXPIRY_DATE");
        $licence_time = $this->getConfig()->getSetting("LICENCE_EXPIRY_TIME");

        $licence_expirty_datetime = $licence_date.' '.$licence_time;
        $converted_strtotime = strtotime($licence_expirty_datetime);
        $currentdate_time = strtotime(Carbon::now()->format(DateFormatHelper::getSystemFormat()));

        if ($currentdate_time > $converted_strtotime) {
            $valid = "Invalid";
        } else {
            $valid = "valid";
        }  
        ...
        // Build a new RF
        $settingFilesXml = new \DOMDocument("1.0");
        $fileElements = $settingFilesXml->createElement("files");
        $settingFilesXml->appendChild($fileElements);
        ...
        // $fileElements->setAttribute('fitlerTo', $this->getDate()->getLocalDate($this->toFilter));
         ...
         // Add the filter dates to the RF xml document
        $fileElements->setAttribute('generated', Carbon::now()->format(DateFormatHelper::getSystemFormat()));
        $fileElements->setAttribute('filterFrom', $this->fromFilter->format(DateFormatHelper::getSystemFormat()));
        $fileElements->setAttribute('filterTo', $this->toFilter->format(DateFormatHelper::getSystemFormat()));



        // Add the file node
        $file = $settingFilesXml->createElement("file");
        $file->setAttribute("volumeDetail", $this->getConfig()->getSetting('VOLUME_SLIDER_VALUE'));
        $file->setAttribute("brightnessDetail", $this->getConfig()->getSetting('BRIGHTNESS_SLIDER_VALUE'));
        $file->setAttribute("downloadstartTime", $this->getConfig()->getSetting('DOWNLOAD_START_TIME'));
        $file->setAttribute("deviceoffDate", $this->getConfig()->getSetting('DEVICE_OFF_DATE'));
        $file->setAttribute("deviceoffTime", $this->getConfig()->getSetting('DEVICE_OFF_TIME'));
        $file->setAttribute("devicevolumeControl", $this->getConfig()->getSetting('VOLUME_SLIDER_VALUE'));
        $file->setAttribute("licenceExpiryDate", $this->getConfig()->getSetting('LICENCE_EXPIRY_DATE'));
        $file->setAttribute("licenceExpiryTime", $this->getConfig()->getSetting('LICENCE_EXPIRY_TIME'));
        $file->setAttribute("deviceLimit", $this->getConfig()->getSetting('DEVICE_LIMIT'));
        $file->setAttribute("deviceLicenceStatus", $valid);
        $file->setAttribute("licenceModule", $this->getConfig()->getSetting('LICENCE_MODULE'));
        
        //$file->setAttribute("securityKeyModule", $this->getConfig()->getSetting('SECURITY_KEY_MODULE'));
        $file->setAttribute("leaveCalendarModule", $this->getConfig()->getSetting('LEAVE_CALENDER_MODULE'));
        $file->setAttribute("apkVersionModule", $this->getConfig()->getSetting('APK_VERSION_MODULE'));
        $file->setAttribute("apkDownloadUrlModule", $this->getConfig()->getSetting('APK_DOWNLOAD_URL_MODULE'));
        $file->setAttribute("volumeSliderModule", $this->getConfig()->getSetting('VOLUME_SLIDER_MODULE'));
        $file->setAttribute("brightnessSliderModule", $this->getConfig()->getSetting('BRIGHTNESS_SLIDER_MODULE'));
        $file->setAttribute("globalMute", $this->getConfig()->getSetting('DEVICE_VOLUME_CONTROL'));
             
        $file->setAttribute("apkVersion", $this->getConfig()->getSetting('APK_VERSION'));
        $file->setAttribute("apkDownloadUrl", $this->getConfig()->getSetting('APK_DOWNLOAD_URL'));
        $fileElements->appendChild($file);


        // Return the results of requiredFiles()
        $settingFilesXml->formatOutput = true;
        $output = $settingFilesXml->saveXML();
